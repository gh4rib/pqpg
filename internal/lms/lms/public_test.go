...
func TestRfc8554KAT2(t *testing.T) {
	...
	sig, err := lms.LmsSignatureFromBytes(lms_signature_bytes)
	if err != nil {
		panic(err)
	}
...
func TestSha256192KAT1(t *testing.T) {
	var publicKeyBytes = []byte{
		...
		0x00, 0x00, 0x00, 0x0a,
		// LM-OTS type: LMOTS_SHA256_N24_W8
		0x00, 0x00, 0x00, 0x08,
		// I
	...
	sig, err := lms.LmsSignatureFromBytes(signatureBytes)
	if err != nil {
		t.Fatalf("LmsSignatureFromBytes() = %v", err)
	}
	pk, err := lms.LmsPublicKeyFromBytes(publicKeyBytes)
	if err != nil {
		t.Fatalf("LmsPublicKeyFromBytes() = %v", err)
	}
	result := pk.Verify(messageBytes, sig)
	assert.True(t, result)
}
